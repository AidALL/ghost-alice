...
- Neon MongoDB Green (`#00ed64`) as the singular brand accent is Electric and organic.
- MongoDB Value Serif for hero headlines is Editorial authority at tech scale.
...
- **Teal Active** (`#1eaedb`): Button hover background. It is bright teal.
...
- **Body / UI**: `Euclid Circular A` It is geometric sans-serif workhorse.
...
- **Wide-tracked monospace labels**: Source Code Pro uppercase at 1px–3px letter-spacing creates technical signposts that feel like database field labels. It is systematic, structured, classified.
...
**Shadow Philosophy**: MongoDB's shadow system is unique in that the primary elevation shadow uses `rgba(0, 30, 43, 0.12)`. It is a teal-tinted shadow that carries the forest-dark brand color into the depth system. This means even on white surfaces, shadows feel like they belong to the MongoDB color world rather than being generic neutral black.

## 7. Do's and Don'ts

### Do
- Use `#001e2b` (forest-black) for dark sections. It is not pure black.
...
3. Serif headlines (MongoDB Value Serif) create the editorial authority Do not use for body.
4. Weight 300 body text creates the airy reading experience is Don't default to 400.
5. Source Code Pro uppercase with wide tracking for technical labels. This means the database voice.
