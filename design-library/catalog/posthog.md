...
- Content-heavy editorial layout. This means the site reads like a magazine, not a typical landing page.
- Tailwind CSS + Radix UI + shadcn/ui component architecture

## 2. Color Palette & Roles

### Primary
- **Olive Ink** (`#4d4f46`): Primary text color is a distinctive olive-gray that gives all text a warm, earthy tone.
- **Deep Olive** (`#23251d`): Link text and high-emphasis headings. It is near-black with green undertone.
- **PostHog Orange** (`#F54E00`): Hidden brand accent. It appears only on hover states as a vibrant orange that surprises.

### Secondary & Accent
- **Amber Gold** (`#F7A501`): Secondary hover accent on dark buttons. It is warm gold that pairs with the orange.
...
### Surface & Background
- **Warm Parchment** (`#fdfdf8`): Primary page background. It is warm near-white with yellow-green undertone.
...
- **Warm Tan** (`#d4c9b8`): Featured button backgrounds It is warm tan/khaki for emphasis.
...
- **Sage Placeholder** (`#9ea096`): Placeholder text, disabled states. It is warm sage-green.
- **Sage Border** (`#bfc1b7`): Primary border color. It is olive-tinted gray for all borders.
- **Light Border** (`#b6b7af`): Secondary border, toolbar borders. It is slightly darker sage.

### Semantic & Accent
- **PostHog Orange** (`#F54E00`): Hover text accent. Signals interactivity and brand personality.
- **Amber Gold** (`#F7A501`): Dark button hover accent. It is a warmth signal.
- **Focus Blue** (`#3b82f6` at 50% opacity): Keyboard focus rings Accessibility-only color.
- **Dark Text** (`#111827`): High-contrast link text. It is near-black for important links.

### Gradient System
- No gradients on the marketing site is PostHog's visual language is deliberately flat and warm.
...
- **Code Display**: `Source Code Pro` It uses fallbacks: `Menlo, Consolas, Monaco`.
...
### Buttons
- **Dark Primary**: `#1e1f23` background, white text, 6px radius, `10px 12px` padding. Hover: opacity 0.7 with Amber Gold text. Active: opacity 0.8 with slight scale transform. The main CTA. It is dark and confident.
...
### Cards & Containers
- **Bordered Card**: Warm Parchment (`#fdfdf8`) or white background, 1px `#bfc1b7` border, 4px–6px radius. It is clean and minimal.
...
- **Hover**: Orange text flashes on interactive cards. This is consistent with button behavior.
...
### AI Chat Widget
- Floating PostHog AI assistant with speech bubble is An interactive product demo embedded in the marketing site.
...
- **The single shadow**: The one defined shadow (`0 25px 50px -12px`) is reserved for floating elements such as modals, dropdowns, and popovers. It is a deep, dramatic shadow that creates clear separation when needed.
...
- Flash PostHog Orange (#F54E00) on hover states is It's the hidden brand signature.
...
- Maintain the warm parchment background (#fdfdf8). It is not pure white, never cold.
...
- Remove the orange hover flash is It's a core interaction pattern, not decoration.
...
4. Confirm borders use sage-tinted gray (#bfc1b7) not neutral gray. It is warmth runs through every element.
5. The overall tone should feel like a fun, scrappy startup wiki Do not corporate-polished or sterile.
