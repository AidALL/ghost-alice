...
- [2. Color Palette & Roles](#2-color-palette-roles)
  ...
  - [Light Mode Neutrals (for light theme contexts)](#light-mode-neutrals-for-light-theme-contexts)
  - [Overlay](#overlay)
...
- Inter Variable with `"cv01", "ss03"` globally uses geometric alternates for a cleaner aesthetic.
...
### Background Surfaces
- **Marketing Black** (`#010102` / `#08090a`): The deepest background is the canvas for hero sections and marketing pages. Near-pure black with an imperceptible blue-cool undertone.
...
- **Secondary Surface** (`#28282c`): The lightest dark surface. It is used for hover states and slightly elevated components.

### Text & Content
- **Primary Text** (`#f7f8f8`): Near-white with a barely-warm cast. The default text color. It is not pure white, preventing eye strain on dark backgrounds.
...
- **Quaternary Text** (`#62666d`): The most subdued text covers timestamps, disabled states, and subtle labels.

### Brand & Accent
- **Brand Indigo** (`#5e6ad2`): Primary brand color. It is used for CTA button backgrounds, brand marks, and key interactive surfaces.
...
- Use `#f7f8f8` for primary text. It is not pure `#ffffff`, which would be too harsh.
...
- Don't skip the OpenType features (`"cv01", "ss03"`). It works without them, it's generic Inter, not Linear's Inter.
...
4. Surface elevation via background opacity: `rgba(255,255,255, 0.02 → 0.04 → 0.05)` Do not use solid backgrounds on dark.
5. Brand indigo (`#5e6ad2` / `#7170ff`) is the only chromatic color is Everything else is grayscale.
