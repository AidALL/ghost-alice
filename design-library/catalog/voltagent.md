...
**Key Characteristics:**
- Carbon-black canvas (`#050507`) with warm-gray border containment (`#3d3a39`) It is not cold or sterile.
...
### Primary
- **Emerald Signal Green** (`#00d992`): The core brand energy It is used for accent borders, glow effects, and the highest-signal interactive moments. This is the "power-on" indicator of the entire interface.
- **VoltAgent Mint** (`#2fd6a1`): The button-text variant of the brand green It is slightly warmer and more readable than pure Signal Green, used specifically for CTA text on dark surfaces.
...
- **Cobalt Primary** (`#306cce`): Docusaurus primary dark It is used in documentation contexts for links and interactive focus states.
...
### Surface & Background
- **Abyss Black** (`#050507`): The landing page canvas is a near-pure black with the faintest warm undertone, darker than most "dark themes" for maximum contrast with green accents.
- **Carbon Surface** (`#101010`): The primary card and button background. It is one shade lighter than Abyss, creating a barely perceptible elevation layer. Used across all contained surfaces.
- **Warm Charcoal Border** (`#3d3a39`): The signature containment color. It is not a cold gray but a warm, almost brownish dark tone that prevents borders from feeling harsh against the black canvas.

### Neutrals & Text
- **Snow White** (`#f2f2f2`): The primary text color on dark surfaces It is not pure white (`#ffffff`) but a softened, eye-friendly off-white. The most-used color on the site (1008 instances).
...
- **Warm Parchment** (`#b8b3b0`): Secondary body text is a warm light gray with a slight pinkish undertone that reads as "paper" against the dark canvas.
- **Steel Slate** (`#8b949e`): Tertiary text, metadata, timestamps, and de-emphasized content. It is a cool blue-gray that provides clear hierarchy below Warm Parchment.
...
- **Deep Dramatic Elevation**: `rgba(0, 0, 0, 0.7) 0px 20px 60px` with `rgba(148, 163, 184, 0.1) 0px 0px 0px 1px inset`. It is a heavy, dramatic downward shadow paired with a faint inset slate ring for the most prominent floating elements.
...
- The default interactive element is Unassuming but clearly clickable.
...
- The "powered on" button is Green text on dark surface reads as an active terminal command.
...
- Dashed variant: `1px dashed rgba(79, 93, 117, 0.4)` for workflow/diagram containers. It is visually distinct from solid-border content cards.

### Inputs & Forms
- No explicit input token data extracted. This means the site is landing-page focused with minimal form UI.
...
- Code blocks are treated as primary visual content is Syntax-highlighted with SFMono-Regular.
...
- Functions as the primary CTA is "Install first, read later" developer psychology.
...
- Use the warm gray palette (`#3d3a39`, `#8b949e`, `#b8b3b0`) for borders and secondary text. It is warmth prevents the dark theme from feeling sterile.
- Present code snippets as primary content is They're hero elements, not supporting illustrations.
...
- Pair system-ui for headings with Inter for body text. This means the speed/authority of native fonts combined with the precision of a geometric sans.
...
- Don't introduce warm colors (orange, red, yellow) as decorative accents. The palette is strictly green + warm neutrals on black. It is warm colors are reserved for semantic states (warning, error) only.
...
- No visible art direction changes between breakpoints. This means the same crops are used with proportional scaling.
...
2. Reference specific color names and hex codes is "Use Warm Parchment (#b8b3b0)" not "make it lighter".
3. Use border treatment to communicate elevation: "change the border to 2px solid Emerald Signal Green (#00d992)" for emphasis
4. Describe the desired "feel" alongside measurements is "Compressed and authoritative heading at 36px with line-height 1.11 and -0.9px letter-spacing".
