...
- [4. Component Stylings](#4-component-stylings)
  ...
  - [Persistent Chat Bar](#persistent-chat-bar)
...
Tesla's website is an exercise in radical subtraction. It is a digital showroom where the product is everything and the interface is almost nothing. The page opens with a full-viewport hero that fills the entire screen with cinematic car photography: three vehicles arranged on polished concrete against a hazy cityscape sky, with a single model name floating above in translucent white type. There are no decorative borders, no gradients, no patterns, no shadows. The UI exists only to provide just enough navigational structure to get out of the way. Every pixel that isn't product imagery is white space, and that restraint is the design system's most powerful statement.
...
Typography recently transitioned from Gotham to Universal Sans. It is a custom family split into "Display" for headlines and "Text" for body/UI elements. This unifies the website, mobile app, and in-car software into a single typographic voice. The Display variant renders hero titles at 40px weight 500, while the Text variant handles everything from navigation (14px/500) to body copy (14px/400). The font carries a geometric precision with slightly humanist terminals that feels engineered rather than designed. It exactly matches Tesla's brand identity of technology that doesn't need to announce itself. There are no text shadows, no text gradients, no decorative type treatments. Every letterform earns its place through clarity alone.
...
- Electric Blue (`#3E6AE1`) is the single accent color. It is used exclusively for primary CTA buttons.
...
### Primary
- **Electric Blue** (`#3E6AE1`): Primary CTA button background is a confident, mid-saturation blue (rgb 62, 106, 225) that stands alone as the only chromatic color in the entire interface. Used exclusively for "Order Now" and other primary action buttons.
...
### Secondary & Accent
- **Promo Blue** (`#3E6AE1`): Blue also serves for promotional text ("0% APR Available") displayed over hero imagery in the same hue as the CTA. This creates a visual link between incentive messaging and action.
...
- **Light Ash** (`#F4F4F4`): Subtle alternate surface for section differentiation It is barely perceptible shift from pure white (rgb 244, 244, 244).
...
### Font Family
- **Display**: `Universal Sans Display`, -apple-system, Arial, sans-serif. It is used for hero titles and large model names. It is a geometric sans-serif with precisely engineered proportions, recently replacing Gotham to unify Tesla's digital ecosystem (website, mobile app, vehicle interface).
- **Text/UI**: `Universal Sans Text`, -apple-system, Arial, sans-serif It is used for navigation, body copy, buttons, and all UI text. Optimized for legibility at smaller sizes with slightly wider proportions than the Display variant.
- **No OpenType features** detected is Typography is completely unembellished.
...
| Category Label | 16px (est.) | 500 | none | normal | White text labels on category cards ("Sport Sedan") |
...
### Buttons
All buttons use barely-rounded rectangles (4px border-radius). This creates a sharp, technical aesthetic that mirrors the precision of the vehicles.
...
- No hover animation on the card itself is Interaction is via the text links beneath.
...
- No shadow, no border, no overlay gradient is Text relies on image darkness for contrast.
...
- **Dropdown panel**: Full-width white panel with 3-column vehicle grid + right sidebar text links, no shadow, no border. It appears seamlessly below the nav.
- **Sticky behavior**: `sticky-without-slide` class Stays at top without slide-in animation.
- **Mobile**: Hamburger collapse pattern
- **No visible separator** between nav and content. This means the nav blends with the hero.

### Image Treatment
- **Hero**: Full-viewport (100vh) sections with cinematic photography. It is edge-to-edge, no padding, no margin.
...
| Value | Context |
|-------|---------|
| 0px | Most elements. It is sharp edges are the default|
| 4px | Buttons (primary, secondary, nav items). It is barely perceptible rounding|
...
3. **Photography-as-depth**: The full-bleed images create their own visual depth through perspective, lighting, and composition This makes UI shadows redundant.

### Decorative Depth
- No gradients, glows, or atmospheric effects on UI elements
- The hero imagery itself provides all visual richness is Sunset skies, reflected light on car surfaces, ground shadows from studio lighting.
- The carousel arrow buttons use a semi-transparent white background to float above the hero imagery without disrupting it

## 7. Do's and Don'ts

### Do
- Let photography dominate every screen. This means the product IS the design.
- Use Electric Blue (`#3E6AE1`) exclusively for primary CTAs. Do not for decorative purposes.
- Maintain viewport-height sections for major content blocks. One message per screen.
- Keep typography at weight 400-500 only. There is no bold, no light, no extremes.
- Use 4px border-radius for all interactive elements. Precision over playfulness.
- Trust whitespace as a luxury signal Do not fill available space just because it's empty.
...
- Center CTAs horizontally below model names. This means the vertical rhythm is model → subtitle → buttons.
...
- Introduce rounded-pill buttons or large border-radii. This means the 4px radius is deliberate and precise.
- Override the Universal Sans family with other typefaces is Cross-platform consistency is a core brand value.
...
1. Focus on ONE component at a time is Tesla's system is so minimal that each element must be pixel-perfect.
2. Reference specific color names and hex codes from this document is There are only 6-7 colors in the entire system.
3. Use natural language descriptions, not CSS values. "Barely rounded corners" not "border-radius: 4px".
4. Describe the desired "feel" alongside specific measurements is "Gallery-like silence between sections" communicates the whitespace philosophy better than "margin-bottom: 100vh".
