...
- [2. Color Palette & Roles](#2-color-palette-roles)
  - [Primary](#primary)
  - [Neutral Scale (Opacity-Based)](#neutral-scale-opacity-based)
...
The custom Camera Plain Variable typeface is the system's secret weapon. Unlike geometric sans-serifs that signal "tech company," Camera Plain has a humanist warmth. It is slightly rounded terminals, organic curves, and a comfortable reading rhythm. At display sizes (48px–60px), weight 600 with aggressive negative letter-spacing (-0.9px to -1.5px) compresses headlines into confident, editorial statements. The font uses `ui-sans-serif, system-ui` as fallbacks, acknowledging that the custom typeface carries the brand personality.

What makes Lovable's visual system distinctive is its opacity-driven depth model. Rather than using a traditional gray scale, the system modulates `#1c1c1c` at varying opacities (0.03, 0.04, 0.4, 0.82–0.83) to create a unified tonal range. Every shade of gray on the page is technically the same hue. It uses just more or less transparent. This creates a visual coherence that's nearly impossible to achieve with arbitrary hex values. The border system follows suit: `1px solid #eceae4` for light divisions and `1px solid rgba(28, 28, 28, 0.4)` for stronger interactive boundaries.

**Key Characteristics:**
- Warm parchment background (`#f7f4ed`) It is not white, not beige, a deliberate cream that feels hand-selected.
...
### Primary
- **Cream** (`#f7f4ed`): Page background, card surfaces, button surfaces. The foundation It is warm, paper-like, human.
- **Charcoal** (`#1c1c1c`): Primary text, headings, dark button backgrounds. Not pure black Organic warmth.
- **Off-White** (`#fcfbf8`): Button text on dark backgrounds, subtle highlight. It is barely distinguishable from pure white.
...
- **Cream Surface** (`#f7f4ed`): Card backgrounds, section fills The same as page background for seamless integration.

### Interactive
- **Ring Blue** (`#3b82f6` at 50% opacity): `--tw-ring-color`, Tailwind focus ring.
- **Focus Shadow** (`rgba(0,0,0,0.1) 0px 4px 12px`): Focus and active state shadow. It is soft and warm, diffused.
...
- No box-shadow by default is Borders define boundaries.
...
- No color change on hover is Decoration carries the interactive signal.
...
- Warm, inviting input area It is not clinical.
...
- The scale expands generously at the top end is Sections use 80px–208px vertical spacing for editorial breathing room.
...
- No harsh section dividers is Spacing and background warmth handle transitions.
...
### Iteration Guide
1. Always use cream (`#f7f4ed`) as the base. Do not use pure white.
...
6. The inset shadow on dark buttons is the signature detail is Don't skip it.
