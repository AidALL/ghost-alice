...
- [2. Color Palette & Roles](#2-color-palette-roles)
  ...
  - [Extended Color Spectrum (Podium CDS)](#extended-color-spectrum-podium-cds)
...
The typography system is the other half of Nike's visual identity. Massive uppercase headlines use Nike Futura ND, a custom condensed Futura variant with impossibly tight line-height (0.90). They punch through hero imagery like a typographic shockwave. Below the headlines, the workhorse Helvetica Now family handles everything from navigation to product descriptions with Swiss-precision clarity. This split between expressive display type and functional body type mirrors Nike's brand duality: inspiration meets execution.
...
- Full-bleed photography with no border radius is Imagery fills every available edge.
...
- Shadow-free, border-minimal elevation model is Surface differentiation through grey shifts only.

## 2. Color Palette & Roles

### Primary

- **Nike Black** (`#111111`): The foundation Primary text, button backgrounds, nav text, hero overlays. It is deliberately not pure black (#000000), creating a fractionally softer reading experience.
...
|------|------|--------|-------------|----------------|-------|
| Display | 96px | 500 | 0.90 | none | Nike Futura ND, uppercase, hero headlines |
| Heading 1 | 32px | 500 | 1.20 | none | Helvetica Now Display Medium, section titles |
| Heading 2 | 24px | 500 | 1.20 | none | Helvetica Now Display Medium, subsections |
| Heading 3 | 16px | 500 | 1.50 | none | Helvetica Now Text Medium, card titles |
| Body | 16px | 400 | 1.75 | none | Helvetica Now Text, product descriptions |
| Body Medium | 16px | 500 | 1.75 | none | Helvetica Now Text Medium, emphasized text |
| Link | 16px | 500 | 1.75 | none | Helvetica Now Text Medium, navigation links |
| Link Small | 14px | 500 | 1.86 | none | Helvetica Now Text Medium, footer/utility links |
| Button | 16px | 500 | 1.50 | none | Helvetica Now Text Medium, CTA text |
| Button Small | 14px | 500 | 1.50 | none | Helvetica Now Text Medium, secondary buttons |
| Caption | 14px | 500 | 1.50 | none | Helvetica Now Text Medium, price labels |
| Small | 12px | 500 | 1.50 | none | Helvetica Now Text Medium, timestamps |
| Tiny | 12px | 400 | 1.50 | none | Helvetica Now Text, legal text |
...
- Background: White (`#FFFFFF`) There is no visible card boundary in most cases.
...
Nike's whitespace strategy is deliberately aggressive. It is not in the luxurious, breathing way of a fashion brand, but in a compressed, high-density way that fills every pixel with either content or intentional absence. Product grids use minimal gaps (4-12px) to create a sense of abundance and choice. Section breaks are generous (48-80px) to separate shopping contexts. The overall effect is a store that feels packed with product while remaining navigable. It is like a well-organized athletic superstore.
...
- Use Nike Black (#111111) for all primary text. Do not use pure #000000.
- Keep buttons pill-shaped (30px radius) and limited to primary/secondary variants
- Use full-bleed, edge-to-edge photography for hero sections. There is no border radius on images.
...
- "Create a promotional banner strip with #111111 background, white 12px/500 centered text, and 8px vertical padding is Full width, no border radius".
...
5. If something feels too colorful in the UI, it probably is is Nike keeps UI greyscale.
