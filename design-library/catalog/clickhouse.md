...
**Key Characteristics:**
- Pure black canvas (#000000) with neon yellow-green (#faff69) accent is Maximum contrast.
...
- All links hover to neon yellow-green is Unified interactive signal.
- Inset shadows on select elements creating "pressed into the surface" depth

## 2. Color Palette & Roles

### Primary
- **Neon Volt** (`#faff69`): The signature brand color is a vivid acid yellow-green that's the sole chromatic accent on the black canvas. Used for primary CTAs, accent borders, link hovers, and highlighted moments.
- **Forest Green** (`#166534`): Secondary CTA color is a deep, saturated green for "Get Started" and primary action buttons that need distinction from the neon.
- **Dark Forest** (`#14572f`): A darker green variant for borders and secondary accents.

### Secondary & Accent
- **Pale Yellow** (`#f4f692`): Active/pressed state text color is a softer, more muted version of Neon Volt for state feedback.
...
### Surface & Background
- **Pure Black** (`#000000`): The primary page background. It is absolute black for maximum contrast.
...
- **Hover Gray** (`#3a3a3a`): Button hover state background. It is slightly lighter than Near Black.
...
### Gradient System
- **None in the traditional sense.** ClickHouse uses flat color blocks and high-contrast borders. The "gradient" is the contrast itself is Neon yellow-green against pure black creates a visual intensity that gradients would dilute.
...
- **Uppercase with maximum tracking**: Section overlines use 1.4px letter-spacing. This is wider than most systems. It creates bold structural labels that stand out against the dense dark background.
...
- The eye-catching CTA is Neon on black.
...
- Full-dark page There is no light sections.
...
- Keep everything on pure black (#000000). Do not use dark gray as the page background.
...
- Show performance stats as oversized display numbers is It's the core visual argument.
- Use uppercase with wide letter-spacing (1.4px) for section labels
- Apply Pale Yellow (#f4f692) for active/pressed text states
- Link hovers should ALWAYS shift to Neon Volt is Unified interactive feedback.
...
### Iteration Guide
1. Keep everything on pure black. There is no dark gray alternatives.
2. Neon Volt (#faff69) is for accents and CTAs only Do not large backgrounds.
3. Weight 900 for hero, 700 for headings, 600 for labels, 400-500 for body
4. Active states use Pale Yellow (#f4f692) It is not just opacity changes.
5. All links hover to Neon Volt is Consistent interactive feedback.
