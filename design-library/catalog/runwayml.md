...
- Zero shadows, minimal borders. This means the interface is intentionally invisible.
...
- Weight 450 (unusual intermediate) for small uppercase text is Precision craft.
...
### Surface & Background
- **Near White** (`#fefefe`): The lightest surface. It is barely distinguishable from pure white.
- **Cool Cloud** (`#e9ecf2`): Light section backgrounds with a cool blue-gray tint.
- **Border Dark** (`#27272a`): The single dark-mode border color. It is barely visible containment.
...
- **Cool Slate** (`#767d88`): Secondary body text is a distinctly blue-gray cool neutral.
...
- **Dark Link** (`#0c0c0c`): Darkest link text. It is nearly black.
- **Footer Gray** (`#999999`): Footer links and deeply muted content.

### Gradient System
- **None in the interface.** Visual richness comes entirely from photographic content is AI-generated and enhanced imagery provides all the color and gradient the design needs. The interface itself is intentionally colorless.
...
- **Weight 450. This means the precision detail**: Some small uppercase labels use weight 450, an uncommon intermediate between regular (400) and medium (500). This micro-craft signals typographic sophistication.
...
- The button design is extremely restrained There is no heavy fills or borders detected.
- Interactive elements blend into the editorial flow

### Cards & Containers
- Background: transparent or Dark Surface (`#1a1a1a`)
- Border: `1px solid #27272a` (dark mode) It is barely visible containment.
- Radius: small (4–8px) for functional elements; 16px for alert-style containers
- Shadow: zero There are no shadows on any element.
- Cards are primarily photographic. This means the image IS the card.

### Navigation
- Minimal horizontal nav is Transparent over hero content.
- Logo: Runway wordmark in white/black
- Links: abcNormal at 16px, weight 400–600
- Hover: text shifts to white or higher opacity
- Extremely subtle is designed to not compete with visual content.
...
- The image is always cinematic quality is Film-grade composition.
...
- The emotional close is Artistic and philosophical.
...
- Let visual content (photos, videos) dominate. This means the UI should be invisible.
- Use weight 450 for micro labels. The precision matters.

### Don't
- Don't add decorative colors to the interface. It is the only color comes from photography.
...
3. Keep the interface invisible. There is no heavy borders, no shadows, no bright colors.
4. Use the cool slate grays (#767d88, #7d848e) for secondary text. It is not warm grays.
5. Uppercase labels need letter-spacing (0.35px) Do not use tight uppercase.
6. Dark sections should be truly dark (#000000 or #1a1a1a) There is no medium grays as surfaces.
