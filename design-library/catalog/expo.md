...
- Pill-shaped geometry everywhere is Buttons, tabs, containers, images (24px–9999px radius).
...
### Primary
- **Expo Black** (`#000000`): The absolute anchor It is used for primary headlines, CTA buttons, and the brand identity. Pure black on cool white creates maximum contrast without feeling aggressive.
...
### Secondary & Accent
- **Link Cobalt** (`#0d74ce`): The standard link color is a trustworthy, saturated blue that signals interactivity without competing with the monochrome hierarchy.
- **Legal Blue** (`#476cff`): A brighter, more saturated blue for legal/footer links. It is slightly more attention-grabbing than Link Cobalt.
- **Widget Sky** (`#47c2ff`): A light, friendly cyan-blue for widget branding elements. The brightest accent in the system.
- **Preview Purple** (`#8145b5`): A rich violet used for "preview" or beta feature indicators This creates clear visual distinction from standard content.

### Surface & Background
- **Cloud Gray** (`#f0f0f3`): The primary page background is a cool off-white with the faintest blue-violet tint. It is not warm or sterile. It is precisely technological.
- **Pure White** (`#ffffff`): Card surfaces, button backgrounds, and elevated content containers create a clear "lifted" distinction from Cloud Gray.
...
### Neutrals & Text
- **Slate Gray** (`#60646c`): The workhorse secondary text color (305 instances). It is a cool blue-gray that's authoritative without being heavy.
...
### Semantic & Accent
- **Warning Amber** (`#ab6400`): A warm, deep amber for warning states. It is deliberately not bright yellow, conveying seriousness.
...
- **Input Border** (`#d9d9e0`): Button and form element borders. It is slightly warmer/darker than card borders for interactive elements.
- **Dark Focus Ring** (`#2547d0`): Deep blue for keyboard focus indicators in dark theme contexts.

### Gradient System
- The design is notably **gradient-free** in the interface layer. Visual richness comes from product screenshots, the React universe illustration, and careful shadow layering rather than color gradients. This absence IS the design decision is Gradients would undermine the clinical precision.
...
- **Weight as hierarchy**: 700–900 for display, 600 for headings, 500 for emphasis, 400 for body. The jumps are decisive No ambiguous in-between weights are used.
...
- The understated default is Clean, professional, unheroic.
...
- The maximum-emphasis CTA is Reserved for primary conversion actions.
...
- Shadow Level 1: Whisper (`rgba(0,0,0,0.08) 0px 3px 6px, rgba(0,0,0,0.07) 0px 2px 4px`) It is barely perceptible lift.
...
- Green dot + text is Compact trust signal.
...
2. Reference specific color names and hex codes is "Use Slate Gray (#60646c)" not "make it gray".
3. Use radius values deliberately. 6px for buttons, 8px for cards, 24px for images, 9999px for pills.
4. Describe the "feel" alongside measurements is "Enormous breathing room with 96px section spacing".
