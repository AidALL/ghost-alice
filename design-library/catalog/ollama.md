...
- Zero shadows is Depth comes exclusively from background color shifts and borders.
- Pill-shaped geometry on all interactive elements (buttons, tabs, inputs, tags)
- The Ollama llama as the sole illustration is Black line art, no color.
- Extreme content restraint. This means the homepage is short, focused, and uncluttered.

## 2. Color Palette & Roles

### Primary
- **Pure Black** (`#000000`): Primary headlines, primary links, and the darkest text. It is the only "color" that demands attention.
- **Near Black** (`#262626`): Button text on light surfaces, secondary headline weight.
- **Darkest Surface** (`#090909`): The darkest possible surface. It is barely distinguishable from pure black, used for footer or dark containers.

### Surface & Background
- **Pure White** (`#ffffff`): The primary page background. It is not off-white, not cream, pure white. Button surfaces for secondary actions.
- **Snow** (`#fafafa`): The subtlest possible surface distinction from white It is used for section backgrounds and barely-elevated containers.
...
- The primary action button is Understated, grayscale, always pill-shaped.
...
- Radius: pill-shaped (9999px)
- Secondary action is Visually lighter than Gray Pill.
...
- Maximum emphasis is Black on white.
...
- No borders, no background is Transparent nav on white page.

### Image Treatment
- The Ollama llama mascot is the only illustration is Black line art on white.
...
- Minimal styling is It uses just a bordered 12px-radius container.
...
- **Emptiness as luxury**: The page is remarkably short and sparse There is no feature section overstays its welcome. Each concept gets minimal but sufficient space.
...
### Do
- Use pure white (`#ffffff`) as the page background. Do not off-white or cream.
...
- Keep the palette strictly grayscale. There is no chromatic colors except the blue focus ring.
...
### Don't
- Don't introduce any chromatic color. There is no brand blue, no accent green, no warm tones.
...
- Don't use font weights above 500. There is no bold, no black weight.
...
4. Shadows are always zero Do not add them.
5. Weight is always 400 or 500 Do not use bold weight.
6. If something feels too decorated, remove it is Less is always more for Ollama.
