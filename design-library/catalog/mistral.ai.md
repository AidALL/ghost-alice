...
What makes Mistral distinctive is the complete commitment to a warm color temperature. The signature "block" identity is a gradient system flowing from bright yellow (`#ffd900`) through amber (`#ffa110`) to burnt orange (`#fa520f`). It creates a visual identity that's immediately recognizable. Even the shadows are warm, using amber-tinted blacks instead of cool grays. Combined with dramatic landscape photography in golden tones, the design feels less like a tech company and more like a European luxury brand that happens to build language models.
...
- The Mistral "M" block identity is A gradient from yellow to orange.
- Dramatic landscape photography in warm golden tones
- Uppercase typography used strategically for section labels and CTAs
- Near-zero border-radius is sharp, architectural geometry.
- French-European confidence: bold, warm, declarative

## 2. Color Palette & Roles

### Primary
- **Mistral Orange** (`#fa520f`): The core brand color is a vivid, saturated orange-red that anchors the entire identity. Used for primary emphasis, the brand block, and the highest-signal moments.
...
- **Block Orange** (`#ff8105`): A pure orange used in the gradient block system. It is warmer and less red than Mistral Orange.
...
- **Bright Yellow** (`#ffd900`): The brightest tone in the gradient. It is used at the "top" of the block identity.

### Surface & Background
- **Warm Ivory** (`#fffaeb`): The lightest page background. It is barely tinted with warmth, the foundation canvas.
...
- **Ultra-tight at scale**: Line-heights of 0.95–1.00 at display sizes create text blocks where ascenders nearly touch descenders from the line above This creates dense, poster-like composition.
...
- Minimal styling is Text link as button.
...
- Radius: near-zero It is sharp, architectural corners.
...
### Image Treatment
- Dramatic landscape photography in warm golden tones
- The winding road through golden hills is A recurring visual motif.
...
- The visual DNA of the brand is Recognizable at any size.
...
### Border Radius Scale
- Near-zero: The dominant radius. It is sharp, architectural corners on most elements.
...
- Use Mistral Black (#1f1f1f) for text. Do not use pure #000000.
...
- Don't add gradients outside the warm spectrum. There is no blue-to-purple, no cool transitions.
...
- No art direction changes. This means the same warm composition is used at all sizes.
...
3. Never add border-radius. It is sharp corners only.
4. Shadows are always warm: "golden shadow with amber tones"
5. Font weight is always 400 is Describe emphasis through size and color.
