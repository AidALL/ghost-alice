...
- [2. Color Palette & Roles](#2-color-palette-roles)
  - [Primary](#primary)
  - [Swatch Palette. It is named Colors.](#swatch-palette-it-is-named-colors)
  - [Neutral Scale (Warm)](#neutral-scale-warm)
...
- Roobert font with 5 OpenType stylistic sets is Quirky geometric character.
...
### Swatch Palette. It is named Colors.
...
### Surface & Border
- **Oat Border** (`#dad4c8`): Primary border. It is warm, cream-toned structural lines.
...
| Badge | Roobert | 9.6px | 600 | none | none | Pill badges |
...
**Shadow Philosophy**: Clay's shadow system is uniquely three-layered: a downward cast (`0px 1px 1px`), an upward inset highlight (`0px -1px 1px inset`), and a subtle edge (`0px -0.5px 1px`). This creates a "pressed into clay" quality where elements feel both raised AND embedded. It is like a clay tablet where content is stamped into the surface. The hover hard shadow (`-7px 7px`) is deliberately retro-graphic, referencing print-era drop shadows and adding physical playfulness.
...
- Use warm oat borders (`#dad4c8`). It is not neutral gray.
...
### Iteration Guide
1. Start with warm cream (#faf9f7). Do not cool white.
2. Swatch colors are for full sections, not small accents is Go bold with matcha, slushie, ube.
3. Oat borders (#dad4c8) everywhere is Dashed variants for decoration.
4. OpenType stylistic sets are mandatory is They make Roobert look like Roobert.
5. Hover animations are the signature is Rotation + hard shadow, not subtle fades.
