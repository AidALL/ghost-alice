...
Coinbase's website is a clean, trustworthy crypto platform that communicates financial reliability through a blue-and-white binary palette. The design uses Coinbase Blue (`#0052ff`). It is a deep, saturated blue. As the singular brand accent against white and near-black surfaces. The proprietary font family includes CoinbaseDisplay for hero headlines, CoinbaseSans for UI text, CoinbaseText for body reading, and CoinbaseIcons for iconography. It is a comprehensive four-font system.
...
- 1.00 line-height on display headings is Ultra-tight.
- Cool gray secondary surface (`#eef0f3`) with blue tint
- `text-transform: lowercase` appears on some button labels. It is unusual.
