...
**Key Characteristics:**
- Warm dark background. It is not cold black, but earthy near-black with warm gray undertones.
- Warm Parchment (`#faf9f6`) text instead of pure white It is subtle cream warmth.
- Matter font family (Regular weight) is geometric but approachable, not the typical developer-tool typeface.
- Nature photography interleaved with product screenshots is Lifestyle meets developer tool.
- Almost monochromatic warm gray palette. There is no bold accent colors.
- Uppercase labels with wide letter-spacing (2.4px) for categorization is Editorial signaling.
- Pill-shaped dark buttons (`#353534`, 50px radius): restrained, muted CTAs.

## 2. Color Palette & Roles

### Primary
- **Warm Parchment** (`#faf9f6`): Primary text color is a barely-cream off-white that softens every surface.
- **Earth Gray** (`#353534`): Button backgrounds, dark interactive surfaces It is warm, not cold.
- **Deep Void** (near-black, page background): The warm dark canvas derived from the body background

### Secondary & Accent
- **Stone Gray** (`#868584`): Secondary text, muted descriptions It is warm mid-gray.
...
### Semantic & Accent
- Warp operates as an almost monochromatic system. There is no bold accent colors.
...
### Font Family
- **Display & Body**: `Matter Regular` It is geometric sans-serif with soft character. Fallbacks: `Matter Regular Placeholder`, system sans-serif.
...
- **UI Supplement**: `Inter` is used for specific UI elements. Fallbacks: `Inter Placeholder`.
- **Monospace Display**: `Geist Mono` For code/terminal display headings.
- **Monospace Body**: `Matter Mono Regular` It is custom mono companion. Fallbacks: `Matter Mono Regular Placeholder`.
...
### Buttons
- **Dark Pill**: `#353534` background, Ash Gray (`#afaeac`) text, pill shape (50px radius), `10px` padding. The primary CTA. It is warm, muted, understated.
...
- **Hover**: Subtle opacity or brightness shift There is no dramatic color changes.
...
### Border Radius Scale
- **4px**: Small interactive elements. It includes buttons and tags.
...
### Shadow Philosophy
Warp's elevation system is remarkably flat. It is almost zero shadow usage on the marketing site. Depth is communicated through:
- **Semi-transparent borders** instead of shadows is Borders at 35% opacity create a ghostly containment.
- **Photography layering** means images create natural depth without artificial shadows.
- **Surface opacity shifts** is `Rgba(255, 255, 255, 0.04)` overlays create barely-perceptible layer differences.
- The effect is calm and grounded is Nothing floats, everything rests.
...
- Interleave nature photography with product screenshots is This is core to the brand identity.
- Maintain the almost monochromatic warm gray palette. There is no bold accent colors.
...
- No art direction changes. This means the same compositions are used across breakpoints.
...
- "Build a category label using Matter Regular at 12px, uppercase transform, letter-spacing 2.4px, stone gray (#868584) color is Editorial magazine style".
...
2. Ensure all buttons use the restrained dark palette (#353534). There is no bright or colorful CTAs.
...
5. The overall tone should feel warm and calm, like a well-designed magazine It is not aggressive or tech-flashy.
