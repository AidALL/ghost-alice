...
- [4. Component Stylings](#4-component-stylings)
  ...
  - [Testimonial / Social Proof](#testimonial-social-proof)
...
- Warm Cream (`#e9e5dd`) buttons instead of bright/saturated CTAs is Understated luxury.
- Lavender Purple (`#cbb7fb`) as the singular accent color is A soft, approachable purple.
- Minimal border-radius scale: only 8px and 16px There is no micro-rounding, no pill shapes.
- Product screenshots dominate the content. This means the UI sells itself with minimal surrounding decoration.
...
- **Charcoal Ink** (`#292827`): Primary text and heading color on light surfaces It is warm near-black with faint brown undertone.

### Secondary & Accent
- **Amethyst Link** (`#714cb6`): Underlined link text. It is mid-range purple that connects to the brand palette while signaling interactivity.
...
### Surface & Background
- **Pure White** (`#ffffff`): Primary page background is the dominant canvas color for all content sections.
- **Warm Cream** (`#e9e5dd`): Button background is a warm, neutral cream that avoids the coldness of pure gray.
- **Parchment Border** (`#dcd7d3`): Card and divider borders. It is warm light gray with slight pink undertone.
...
### Semantic & Accent
- Superhuman operates with extreme color restraint is Lavender Glow (`#cbb7fb`) is the only true accent.
...
- No other gradients on the marketing site. This means the hero gradient is a singular dramatic gesture.

## 3. Typography Rules

### Font Family
- **Display & Body**: `Super Sans VF` It is custom variable font with non-standard weight axis. Fallbacks: `system-ui, -apple-system, Segoe UI, Roboto, Oxygen, Ubuntu, Cantarell, Fira Sans, Droid Sans, Helvetica Neue`.
- **Product UI** (referenced in brand): `Messina Sans` / `Messina Serif` / `Messina Mono` from Luzi Type It is used in the product itself for sans-serif-to-serif transitions.
...
### Principles
- **Non-standard weight axis**: Weights 460 and 540 are deliberately between conventional Regular (400) and Medium (500), creating a typographic texture that feels subtly "off" in a confident way It is slightly heavier than expected, never quite bold.
...
### Buttons
- **Warm Cream Primary**: `#e9e5dd` background, Charcoal Ink (`#292827`) text, subtle rounded corners (8px radius), no visible border. The signature CTA. It is warm, muted, luxurious rather than aggressive.
- **Dark Primary** (on light sections): `#292827` background with white text, 8px radius. It is the inverse of the warm cream for contrast sections.
- **Ghost / Text Link**: No background, underline decoration, Amethyst Link (`#714cb6`) or Charcoal Ink color depending on context
- **Hero CTA**: Warm Cream on the dark purple gradient. It is the cream color pops dramatically against `#1b1938`.
- **Hover**: Subtle opacity or brightness shift There is no dramatic color transformations.

### Cards & Containers
- **Content Card**: White background, Parchment Border (`#dcd7d3`) 1px border, 16px border-radius. It is clean and minimal.
...
### Inputs & Forms
- Minimal form presence on the marketing site is Superhuman funnels users directly to signup.
...
- **CTA button**: Warm Cream (`#e9e5dd`) pill in the nav It is subtle, not attention-grabbing.
...
- Feature descriptions paired with product screenshots is Proof through demonstration rather than quotes.
...
- Only two radii in the entire system is Radical simplicity. No micro-rounding (2px), no pill shapes (50px+).
...
- Use Warm Cream (`#e9e5dd`) for primary buttons. It is not white, not gray, specifically warm cream.
- Limit border-radius to 8px (small) and 16px (large). This means the binary radius system is deliberate.
...
- Let product screenshots be the primary visual content. This means the UI sells itself.
...
- Introduce additional accent colors beyond Lavender Glow. This means the palette is deliberately restrained to one accent.
...
- Hero silhouette image crops or scales is Maintains dramatic composition.
- No art direction changes. This means the same compositions across all breakpoints.
...
4. Confirm the only accent color is Lavender Glow (#cbb7fb). There is no other hues should appear.
5. The overall tone should feel like a luxury product presentation is Minimal, confident, with one dramatic color gesture in the hero.
