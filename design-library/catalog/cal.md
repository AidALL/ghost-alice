...
Cal.com's website is a masterclass in monochromatic restraint. It is a grayscale world where boldness comes not from color but from the sheer confidence of black text on white space. Inspired by Uber's minimal aesthetic, the palette is deliberately stripped of hue: near-black headings (`#242424`), mid-gray secondary text (`#898989`), and pure white surfaces. Color is treated as a foreign substance. When it appears (a rare blue link, a green trust badge), it feels like a controlled accent in an otherwise black-and-white photograph.
...
**Key Characteristics:**
- Purely grayscale brand palette. There are no brand colors, boldness through monochrome.
...
- Cal Sans for headings, Inter for body is Clean typographic division.
- Wide border-radius scale from 2px to 9999px (pill): versatile rounding.
- White canvas with near-black (#242424) text is Maximum contrast, zero decoration.
- Product screenshots as primary visual content. This means the scheduling UI sells itself.
...
- **Midnight** (`#111111`): Deepest text/overlay color. It is used at 50% opacity for subtle overlays.
...
- **Default Link** (`#0000ee`): Browser-default link color on some elements. It is unmodified, signaling openness.

### Surface & Background
- **Pure White** (`#ffffff`): Primary page background and card surfaces
- **Light Gray** (approx `#f5f5f5`): Subtle section differentiation It is barely visible tint.
...
### Semantic & Accent
- Cal.com is deliberately colorless for brand elements is "A grayscale brand to emphasise on boldness and professionalism".
...
### Gradient System
- No gradients on the marketing site. This means the design is fully flat and monochrome.
- Depth is achieved entirely through shadows, not color transitions

## 3. Typography Rules

### Font Family
- **Display**: `Cal Sans` It is custom geometric sans-serif by Mark Davis. Open-source, available on Google Fonts and GitHub. Extremely tight default letter-spacing designed for large headlines. Has 6 character variants (Cc, j, t, u, 0, 1).
- **Body**: `Inter` "Rock-solid" standard body font. Fallback: `Inter Placeholder`.
- **UI Light**: `Cal Sans UI Variable Light` is a light-weight variant (300) for softer UI text with -0.2px letter-spacing.
- **UI Medium**: `Cal Sans UI Medium` is a medium-weight variant (500) for emphasized captions.
...
### Principles
- **Cal Sans at large, Inter at small**: Cal Sans is exclusively for headings and display. Do not for body text. The system enforces this division strictly.
...
- **8px**: Standard interactive elements. It includes buttons, inputs, and images.
...
- Shadows are composed in comma-separated stacks is Each surface gets 2-3 layered shadow definitions working together.

### Decorative Depth
- No gradients or glow effects
- All depth comes from the sophisticated shadow compositing system
- The overall effect is subtle but precise is Surfaces feel like physical cards sitting on a table.
...
- Let product screenshots be the visual content. There are no illustrations, no decorative graphics.
...
- Mix Cal Sans weights. This means the font is designed for weight 600, other weights break the intended character.
- Reduce section spacing below 48px. This means the generous whitespace is core to the premium monochrome aesthetic.
...
- No art direction changes. This means the same compositions are used at all sizes.
...
1. Verify headings use Cal Sans at weight 600, body uses Inter. Do not mix them.
...
5. The overall tone should feel like a clean, professional scheduling tool is Monochrome confidence without any decorative flourishes.
