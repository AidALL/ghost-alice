...
- [2. Color Palette & Roles](#2-color-palette-roles)
  ...
  - [Neutrals & Text (Zinc Scale)](#neutrals-text-zinc-scale)
...
- Zinc-based neutral scale for text hierarchy is Balanced between warm and cool.
...
- **Neon Green** (`#36F4A4`): The signature accent. Focus rings, interactive highlights, active state indicators. It is electric and bioluminescent.
...
- **Void** (`#000000`): Root page background. It is true black for maximum depth.
- **Deep Teal** (`#02090A`): Card surfaces, content containers. It is near-black with green undertone.
...
- **Dark Teal Wash**: Radial gradient from `#102620` center to `#02090A` edge It is used behind product showcases.
...
|------|------|--------|-------------|----------------|-------|
| Display XL | 96px | 400 | 1.00 | none | NeueHaasGrotesk, hero headlines, "ss03" |
...
| Display Light | 96px | 330 | 0.96 | none | NeueHaasGrotesk, ethereal display |
| Heading 1 | 70px | 330 | 1.00 | none | NeueHaasGrotesk, section titles |
| Heading 2 | 55px | 330 | 1.16 | none | NeueHaasGrotesk, subsections |
| Heading 3 | 48px | 330 | 1.14 | none | NeueHaasGrotesk, feature titles |
...
| Body | 18px | 400 | 1.56 | none | Inter-Variable, standard body |
| Body Medium | 18px | 550 | 1.56 | none | Inter-Variable, emphasized body |
| Body Small | 16px | 400 | 1.50 | none | Inter / NeueHaasGrotesk, compact body |
| Body Small Medium | 16px | 420 | 1.50 | none | Inter-Variable, slightly emphasized |
| Button | 16px | 400 | 1.50 | none | NeueHaasGrotesk, CTA text |
...
| Code | 16px | 400 | 1.50 | none | ui-monospace, uppercase, code blocks |
| Code Small | 12px | 400 | 1.33 | none | ui-monospace, uppercase, inline code |
...
- Border: 1px solid `#1E2C31` (Dark Card Border) It is barely visible boundary.
...
- All images sit flush within dark containers. There is no bright borders or frames.
...
- Use Inter Variable for body text and NeueHaasGrotesk for headings. Do not mix their roles.
...
- "Design a tag/badge with rgba(255,255,255,0.2) frosted glass background, 4px radius, 12px 16px padding, white 16px text is Floating over a dark card surface".
...
7. Shadows are multi-layered is A single `box-shadow` value won't capture the Shopify card feel.
