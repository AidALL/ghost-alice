...
Framer's website is a cinematic, tool-obsessed dark canvas that radiates the confidence of a design tool built by designers who worship craft. The entire experience is drenched in pure black. It is not a warm charcoal or a cozy dark gray, but an absolute void (`#000000`) that makes every element, every screenshot, every typographic flourish feel like it's floating in deep space. This is a website that treats its own product UI as the hero art, embedding full-fidelity screenshots and interactive demos directly into the narrative flow.

The typography is the signature move: GT Walsheim with aggressively tight letter-spacing (as extreme as -5.5px on 110px display text) creates headlines that feel compressed, kinetic, almost spring-loaded. It is like words under pressure that might expand at any moment. The transition to Inter for body text is seamless, with extensive OpenType feature usage (`cv01`, `cv05`, `cv09`, `cv11`, `ss03`, `ss07`) that gives even small text a refined, custom feel. Framer Blue (`#0099ff`) is deployed sparingly but decisively. As link color, border accents, and subtle ring shadows. This creates a cold, electric throughline against the warm-less black.
...
**Key Characteristics:**
- Pure black (`#000000`) void canvas is Absolute dark, not warm or gray-tinted.
- GT Walsheim display font with extreme negative letter-spacing (-5.5px at 110px)
- Framer Blue (`#0099ff`) as the sole accent color is Cold, electric, precise.
- Pill-shaped buttons (40px–100px radius) There is no sharp corners on interactive elements.
- Product screenshots as hero art. This means the tool IS the marketing.
...
### Gradient System
- No prominent gradient usage is Framer relies on pure flat black surfaces with occasional blue-tinted glows for depth.
- Subtle radial glow effects behind product screenshots using Framer Blue at very low opacity

## 3. Typography Rules

### Font Family
- **Display**: `GT Walsheim Framer Medium` / `GT Walsheim Medium` It is custom geometric sans-serif, weight 500. Fallbacks: `GT Walsheim Framer Medium Placeholder`, system sans-serif.
...
- **Weight restraint on display**: All GT Walsheim usage is weight 500 (medium) Do not use bold weight, never regular. This creates a confident-but-not-aggressive display tone.
...
- **Solid White Pill**: `rgb(255, 255, 255)` background, black text (`#000000`), full pill shape (100px radius), padding `10px 15px`. The primary CTA. It is clean, high-contrast on dark, unmissable.
...
- Minimal ornamentation. This means the product screenshots serve as the trust signal.
...
- **8px**: Standard component radius for code blocks, buttons, and interactive elements.
- **10px–12px**: Cards, product screenshots. It is comfortably rounded.
- **15px–20px**: Large containers, feature cards. It is generously rounded.
...
| Level 3 (Floating) | `rgba(255, 255, 255, 0.1) 0px 0.5px 0px 0.5px, rgba(0, 0, 0, 0.25) 0px 10px 30px` | Elevated cards, floating elements. It is subtle white top-edge highlight + deep ambient shadow|
...
- **Blue-tinted ring shadows** at very low opacity (0.15) for containment is A signature move that subtly brands every bordered element.
- **White edge highlights** (0.5px) on the top edge of elevated elements is Simulating light hitting the top surface.
- **Deep ambient shadows** for true floating elements use `Rgba(0, 0, 0, 0.25)` at large spread (30px).
...
### Do
- Use pure black (`#000000`) as the primary background. It is not dark gray, not charcoal.
- Apply extreme negative letter-spacing on GT Walsheim display text (-3px to -5.5px)
- Keep all buttons pill-shaped (40px+ radius). Do not use squared or slightly-rounded buttons.
...
- Let product screenshots be the visual centerpiece. This means the tool markets itself.
...
- Introduce additional accent colors beyond Framer Blue is This is a one-accent-color system.
...
- Place light/white backgrounds behind content sections. This means the void is sacred.
...
- No art direction changes. This means the same crops are used across breakpoints.
...
1. Focus on ONE component at a time. This means the dark canvas makes each element precious.
2. Always verify letter-spacing on GT Walsheim headings. The extreme negative tracking is non-negotiable.
3. Check that Framer Blue appears ONLY on interactive elements. Do not as decorative background or text color for non-links.
