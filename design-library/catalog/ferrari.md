...
- **Deep Red** (`#9D2211`): The most saturated dark red It is used for active states and extra emphasis where even Dark Red needs more weight (--f-color-accent-80).
...
- **Modena Yellow** (`#F6E500`): Slightly warmer and more golden than Racing Yellow It is used for secondary heritage accents and category markers (--f-color-yellow).
...
- **Dark Surface** (`#303030`): Secondary dark surface for footer regions, newsletter sections, and layered dark panels It is slightly lifted from pure black for depth differentiation (--f-color-ui-90).
...
### Neutrals & Text
- **Near Black** (`#181818`): Primary body text color on light surfaces It is slightly softened from absolute black for better readability (link default color).
- **Dark Gray** (`#666666`): Secondary text and subdued UI labels. It is used where text needs to recede from the primary hierarchy (--f-color-black-60).
...
- **Link Hover Blue** (`#3860BE`): Interactive hover state for text links is a dignified navy-blue that signals interactivity without competing with Ferrari Red.
...
### Font Family
- **FerrariSans**: Primary typeface for headings, navigation, buttons, and editorial content. It is a proprietary sans-serif with medium weight as the default (500), compact x-height, and precise letter-spacing control. Fallbacks: Arial, Helvetica, sans-serif
...
- The only button that uses Ferrari Red is Reserved for maximum visual priority.
...
- No border, no shadow. This means the darkness IS the container.
...
### Navigation
- **Desktop**: Prancing Horse logo centered at top of page, primary navigation below It is not a traditional horizontal nav bar but a full-width header block on black background.
...
- **No sticky nav behavior** observed. This means the page scrolls naturally with the header moving off-screen.
...
- **Image quality**: High-resolution photography with studio lighting There is no user-generated or lifestyle imagery. Every image is art-directed.
...
| 2px | Default for buttons, inputs, and interactive elements. It is barely perceptible, razor-precision|
...
- The Prancing Horse logo on black creates a "floating in void" effect through pure contrast There is no glow or shadow needed.
- Dark-to-light section transitions are hard cuts, not gradient blends is Reinforcing the editorial page-turn metaphor.
...
- Use the Prancing Horse emblem as a standalone hero element on black. Do not crowd it with adjacent content.
...
### Don't
- Scatter Ferrari Red across the interface as decoration is It's a CTA signal, not a theme color.
...
- Mix FerrariSans and Body-Font within the same text block is They serve separate hierarchical functions.
...
- Override the semantic color system (warning, success, info) with brand colors is `#F13A2C` warning is deliberately different from `#DA291C` brand red.
...
- "Build an editorial card on white background with a full-width image (16:9 ratio) above, a FerrariSans heading (16px/700, Near Black #181818) below, and a Body-Font uppercase label (11px, 1px letter-spacing, Mid Gray #8F8F8F) as the category tag There is no border, no shadow, no border-radius".
...
1. Focus on ONE component at a time is Ferrari's editorial rhythm means each section is a self-contained vignette.
2. Reference specific color names and hex codes from this document. This means the palette is small but each color has a precise role.
3. Use natural language descriptions, not CSS values. "Razor-sharp 2px corners" conveys intent better than "border-radius: 2px".
4. Describe the desired "feel" alongside specific measurements is "Editorial magazine page-turn between sections" communicates the layout philosophy better than "margin-bottom: 80px".
