...
class InstallCmdWrapperTest(unittest.TestCase):
    ...
    def test_powershell_help_exposes_official_addon_alias_and_addon_tag_contract(self) -> None:
        ...
        self.assertIn("--addon autopilot", help_text)
        self.assertIn("--addon-source PATH", help_text)
        self.assertIn("--update-source", help_text)
        self.assertNotIn("-Platform", help_text)
        self.assertNotIn("-AddonSource", help_text)
        self.assertNotIn("-UpdateSource", help_text)
    ...
    def test_powershell_accepts_documented_hyphenated_long_flags(self) -> None:
        pwsh = shutil.which("pwsh") or shutil.which("powershell")
        if not pwsh:
            self.skipTest("PowerShell executable is required for long flag binding test")

        cases = (
            ("--prompt-platform",),
            ("--addon-source", r"C:\tmp\addon"),
            ("--addon-tag", "main"),
            ("--addon-skip",),
            ("--list-addons",),
            ("--cleanup-pending",),
